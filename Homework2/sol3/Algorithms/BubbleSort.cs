...
public class BubbleSort<T> : ISorter<T> where T : IComparable<T> // to use CompareTo
{
    private IEnumerable<T> _collection;
    private IComparer<T>? _comparer; // ? to take nulls

    public BubbleSort(IEnumerable<T> collection)
    {
        _collection = collection;
        _comparer = null;
    }

    public BubbleSort(IEnumerable<T> collection, IComparer<T> comparer)
    {
        _collection = collection;
        _comparer = comparer;
    }
